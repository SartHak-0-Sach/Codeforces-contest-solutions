...
int main() {
	...
	struct seg_t {
		int64_t initial_keep = 0;
		int64_t initial_flow = 0;
		int64_t max_keep = 0;
		int64_t max_flow = 0;
	};
	std::vector<seg_t> seg(2*layout.N);
	auto update_node = [&](seg_tree::point a) {
		...
		if (a < layout.N) {
			...
			int i = layout.get_leaf_index(a);
			// We'll make l the source
			l.initial_flow = A[i];
			l.initial_keep = 0;
			l.max_keep = 0;
			l.max_flow = INF;
		...
		s.initial_keep = l.initial_keep + r.initial_keep;
		s.initial_flow = r.initial_flow;

		// First process l.initial_flow
		{
			int64_t v = std::min(l.initial_flow, r.max_keep);
			l.initial_flow -= v;
			r.max_keep -= v;
			s.initial_keep += v;
		}
		{
			int64_t v = std::min(l.initial_flow, r.max_flow);
			l.initial_flow -= v;
			r.max_flow -= v;
			s.initial_flow += v;
		}

		// Now, any additional flow must pass through l and then r
		s.max_keep = l.max_keep + std::min(l.max_flow, r.max_keep);
		s.max_flow = std::min(r.max_flow, l.max_flow - std::min(l.max_flow, r.max_keep));

		//cerr << "node " << a << ' ' << layout.get_node_bounds(a)[0] << '-' << layout.get_node_bounds(a)[1] << ": " << s.initial_keep << ' ' << s.initial_flow << ' ' << s.max_keep << ' ' << s.max_flow << '\n';
	};
	...
	for (int q = 0; q < Q; q++) {
		int p; cin >> p; p--;
		cin >> A[p] >> B[p] >> C[p];
		auto a = layout.get_point(p);
		update_node(a);
		a.for_parents_up(update_node);
		cout << seg[1].initial_keep << '\n';
	}
